...
final class UpdateChecker {
    ...
    private func installUpdate(from zipFile: URL) {
        ...
        let tempDir = fm.temporaryDirectory.appendingPathComponent("BatteryManager-update-\(UUID().uuidString)")
...
private class DownloadDelegate: NSObject, URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        let dest = FileManager.default.temporaryDirectory.appendingPathComponent("BatteryManager-\(UUID().uuidString).zip")
